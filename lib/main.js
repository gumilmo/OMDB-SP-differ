...
const differ_1 = require("./services/differ");
...
const source = new comparable_document_model_1.ComparableDocument(loadFile("D://OMDB-SP-differ/test-pages/1-src.html").toString().split('\n').map((line, index) => new line_model_1.Line(line.replace('\r', ''), index + 1)));
const dest = new comparable_document_model_1.ComparableDocument(loadFile("D://OMDB-SP-differ/test-pages/1-dst.html").toString().split('\n').map((line, index) => new line_model_1.Line(line.replace('\r', ''), index + 1)));
const test = new differ_1.Differ(source, dest);
