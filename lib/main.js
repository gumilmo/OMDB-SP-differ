...
const sourceFileJSdom = new jsdom_1.JSDOM((0, fs_1.readFileSync)("././test-pages/10-src.html").toString());
const destFileJSdom = new jsdom_1.JSDOM((0, fs_1.readFileSync)("././test-pages/10-dst.html").toString());
...
const serv = new differ_dom_serivce_1.DifferDomSerivce(SourceBody, DestBody);
serv.DOMHandler();
...
if (options.compare) {
    ...
    createResultHtml(html_generator_service_1.HtmlGeneratorService.createHtmlView(lines, timeAppStart, timeAppEnd, paths[0], paths[1]), lines, timeAppEnd);
}
else {
    program.help();
}
